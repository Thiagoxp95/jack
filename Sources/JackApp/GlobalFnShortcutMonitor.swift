...
final class GlobalFnShortcutMonitor {
    ...
    deinit {
        // Ensure the CGEventTap is torn down so stale taps don't keep running
        // after a DictationController recreation.
    ...
    private func evaluateModifierShortcut(_ shortcut: InvocationShortcut, keyCode: Int64, isActive: Bool) -> Bool? {
        ...
        if shortcut.primaryKeyCode == nil {
            ...
            // Mask to device-independent bits so device-specific flags don't
            // cause spurious mismatches.
            let mask = NSEvent.ModifierFlags.deviceIndependentFlagsMask
            let required = shortcut.nsModifierFlags.intersection(mask)
            let current = currentModifierFlags.intersection(mask)
            let allHeld = current.contains(required)
            if allHeld != isActive {
                return allHeld
            }
            return nil
