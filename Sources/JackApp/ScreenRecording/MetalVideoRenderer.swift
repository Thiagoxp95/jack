...
final class MetalVideoRenderer {
    ...
    private static let logger = Logger(
        subsystem: "com.jack.app.v2",
        category: "MetalVideoRenderer"
    )

    /// Resolves the Metal shader library across every supported packaging layout:
    /// precompiled `default.metallib` (Xcode builds), a flat `ZoomCursorCompositor.metal`
    /// in the app's Resources (package_app.sh builds), or the SPM resource bundle
    /// (tests / `swift run`). Never calls `Bundle.module` — its generated accessor
    /// `fatalError`s when the bundle is missing, which we've observed on shipped builds.
    private static func loadShaderLibrary(device: MTLDevice) -> MTLLibrary? {
        if let lib = try? device.makeDefaultLibrary(bundle: .main),
           lib.makeFunction(name: "zoomCursorComposite") != nil {
            return lib
        }
        ...
        for bundle in candidateBundles() {
            ...
            do {
                ...
                logger.error("Failed to compile shader at \(url.path): \(error.localizedDescription)")
            }
        }

        logger.error("Could not locate ZoomCursorCompositor shader in any bundle")
        return nil
    }

    /// Bundles that might hold the shader, ordered by likelihood. Built manually so a
    /// missing SPM resource bundle returns nil instead of trapping.
    private static func candidateBundles() -> [Bundle] {
        var seen = Set<String>()
        var result: [Bundle] = []
        let add: (Bundle?) -> Void = { bundle in
            guard let bundle, seen.insert(bundle.bundlePath).inserted else { return }
            result.append(bundle)
        }

        add(Bundle.main)
        add(Bundle(for: MetalVideoRenderer.self))

        let spmBundleName = "JackApp_JackApp"
        let searchRoots: [URL?] = [
            Bundle.main.resourceURL,
            Bundle.main.bundleURL,
            Bundle(for: MetalVideoRenderer.self).resourceURL,
            Bundle(for: MetalVideoRenderer.self).bundleURL.deletingLastPathComponent(),
        ]
        for root in searchRoots.compactMap({ $0 }) {
            let candidate = root.appendingPathComponent("\(spmBundleName).bundle")
            add(Bundle(url: candidate))
        }
