...
final class RecordingSessionController {
    ...
    static let cacheDirectory: URL = {
        let path = ("~/Library/Caches/Jack/recordings" as NSString).expandingTildeInPath
        return URL(fileURLWithPath: path, isDirectory: true)
    }()

    static let exportDirectory: URL = {
        let path = ("~/Documents/Jack Recordings" as NSString).expandingTildeInPath
        return URL(fileURLWithPath: path, isDirectory: true)
    }()
    ...
    // MARK: - Auto-Select Display Under Cursor

    /// Sets `selectedDisplayIndex` to the display that currently contains the mouse cursor.
    /// Falls back to index 0 if no match is found.
    func selectDisplayUnderCursor() {
        ...
        if let index = availableDisplays.firstIndex(where: { $0.displayID == screenNumber }) {
    ...
    func startRecording() async throws {
        ...
        // Resolve the NSScreen matching the selected display so the countdown
        // appears on the correct monitor (not just NSScreen.main).
        let targetScreen: NSScreen? = {
            guard selectedDisplayIndex < availableDisplays.count else { return nil }
            let displayID = availableDisplays[selectedDisplayIndex].displayID
        ...
        await countdownOverlay.show(on: targetScreen)

        // Hide app windows so they don't appear in the recording.
        // The webcam overlay and recording bubble are created *after* this
        // point, so it is safe to hide both .normal and .floating windows.
        ...
        // Mute Slack if the user has the setting enabled
        muteSlackIfNeeded()
    ...
    func restartRecording() async throws {
        ...
        Self.logger.info("Restarting recording — tearing down current take")

        stopElapsedTimer()

        // Stop all capture services (but don't transition to .editing)
        ...
        // Discard the current session directory
        ...
        // Go back to setup so startRecording() accepts the transition
        state = .setup

        // Start a fresh recording with the same config
    ...
    func stopRecording() async {
        ...
        // Restore Slack mute state
        restoreSlackIfNeeded()
    ...
    func saveEditing(editor: VideoEditorController, title: String) {
        // Dismiss editor window (already hidden by EditorWindowController)
        recordingBubble.hide()
        countdownOverlay.cleanup()

        guard let session = currentSession else { return }

        // Build export config — file on disk uses timestamp, metadata uses user title
        let cacheDir = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first!
        let exportDir = cacheDir.appendingPathComponent("Jack/exports", isDirectory: true)
        try? FileManager.default.createDirectory(at: exportDir, withIntermediateDirectories: true)
        let timestamp = ISO8601DateFormatter().string(from: Date())
            .replacingOccurrences(of: ":", with: "-")
        let outputURL = exportDir.appendingPathComponent("Recording-\(timestamp).mp4")
        ...
        // Enqueue background export + upload (appears immediately in recordings list)
        UploadQueue.shared.enqueueExport(
            session: session,
            editor: editor,
            config: config,
            title: title,
        ...
        // Don't delete session directory yet — the export job needs it.
        // It will be cleaned up after export completes via the exported file in caches.
        ...
        // Navigate the main window to the recordings tab
        NotificationCenter.default.post(name: .showRecordingsTab, object: nil)

        Self.logger.info("Recording saved, export enqueued, navigating to recordings")
    }
    ...
    private func muteSlackIfNeeded() {
        guard UserDefaults.standard.bool(forKey: "slack_mute_enabled") else { return }
    ...
    private func restoreSlackIfNeeded() {
        let service = slackMuteService
        DispatchQueue.global(qos: .userInitiated).async {
