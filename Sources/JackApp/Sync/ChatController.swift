...
final class ChatController {
    ...
    /// Add a user message to the local messages array optimistically (before server confirms).
    func addOptimisticUserMessage(threadId: String, content: String) {
        let optimistic = ConvexChatMessage(
    ...
    func sendAndStream(threadId: String, content: String) async {
        ...
        streamTask = Task { [weak self] in
            ...
            // Capture streamed content before clearing it
            let finalContent = self?.streamedContent ?? ""

            // Save the assistant message to the database from the client side
            if !finalContent.isEmpty {
                let model = self?.threads.first(where: { $0.id == threadId })?.model ?? "unknown"
                do {
                    let token = try await ConvexHTTPClient.getToken()
                    _ = try await ConvexHTTPClient.mutation(
                        function: "chats:saveAssistantMessage",
                        args: [
                            "threadId": threadId,
                    ...
                    NSLog("[ChatController] Failed to save assistant message: %@", String(describing: error))
                }
            }
