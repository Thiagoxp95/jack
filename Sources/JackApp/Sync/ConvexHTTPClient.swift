...
enum ConvexHTTPClient {
    ...
    /// Whether `syncUser` has been called successfully this session.
    @MainActor private static var userSynced = false

    /// Ensure the current user exists in Convex DB by calling `syncUser` via HTTP.
    /// Idempotent — only calls the mutation once per session. Call this before any
    /// mutation that requires a user record (uploads, todos, chats, etc.).
    @MainActor
    static func ensureUserSynced() async throws {
        guard !userSynced else { return }

        let token = try await getToken()
        for attempt in 1...3 {
            do {
                _ = try await mutation(
                    function: "users:syncUser",
                    token: token
                )
                userSynced = true
                NSLog("[ConvexHTTPClient] syncUser succeeded on attempt %d", attempt)
                return
            } catch {
                NSLog("[ConvexHTTPClient] syncUser attempt %d failed: %@", attempt, String(describing: error))
                if attempt < 3 {
                    try? await Task.sleep(for: .seconds(1))
                }
            }
        }
        throw Error.serverError("syncUser failed after 3 attempts")
    }
