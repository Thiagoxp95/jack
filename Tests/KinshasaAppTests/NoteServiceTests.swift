...
final class NoteServiceTests: XCTestCase {
    func testAppendVoiceNoteCreatesDailyMarkdownFile() throws {
        let tempRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("actionfy-note-tests-\(UUID().uuidString)", isDirectory: true)
        let notesDir = tempRoot.appendingPathComponent("Actionfy Notes", isDirectory: true)
        ...
        XCTAssertTrue(contents.contains("# Actionfy Voice Notes (2026-02-20)"))
